  ...
  return (
    <div className="relative" ref={ref}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-800/60 px-3 py-2 text-sm text-slate-200 hover:border-slate-600 hover:bg-slate-800 light:border-slate-300 light:bg-white light:text-slate-700 light:hover:border-slate-400"
      >
        <span>{label}</span>
        {selected.length > 0 && (
          <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-indigo-500/90 px-1 text-xs font-semibold text-white">
            {selected.length}
          </span>
        )}
